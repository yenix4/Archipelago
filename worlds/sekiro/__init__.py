...
from .Bosses import SekiroBossInfo, all_bosses, default_yhorm_location
from .Items import SekiroItem, SekiroItemData, Infusion, filler_item_names, item_descriptions, item_dictionary, item_name_groups
...
class SekiroWorld(World):
    ...
    yhorm_location: SekiroBossInfo = default_yhorm_location
    ...
    def _allow_boss_for_yhorm(self, boss: SekiroBossInfo) -> bool:
    ...
    def stage_post_fill(cls, multiworld: MultiWorld):
        ...
        """
        ds3_worlds = [world for world in cast(List[SekiroWorld], multiworld.get_game_worlds(cls.game)) if
        ...
        for ds3_world in ds3_worlds:
            ...
            all_item_order: List[SekiroItemData] = [
            ...
            # All SekiroItems for this world that have been assigned anywhere, grouped by name
            full_items_by_name: Dict[str, List[SekiroItem]] = defaultdict(list)
            ...
            def smooth_items(item_order: List[Union[SekiroItemData, SekiroItem]]) -> None:
                ...
                # Convert items to full SekiroItems.
                converted_item_order: List[SekiroItem] = [
                    item for item in (
                        (
                            # full_items_by_name won't contain DLC items if the DLC is disabled.
                            (full_items_by_name[item.name] or [None]).pop(0)
                            if isinstance(item, SekiroItemData) else item
                        )
